...
namespace Crawler
{
    public class InvertList
    {
        ...
        private int docNum;
        public InvertList(string domain, string parentForderPath)
        {
        ...
        public void Invert()
        {
            ...
            var distList = new Dictionary<string, IList<int>>();
            foreach (var item in dict)
            {
                distList[item.Key] = item.Value.Distinct().ToList();
                //dict[item.Key] = item.Value.Distinct().ToList();
            }

            var invertDict = JsonConvert.SerializeObject(distList);
